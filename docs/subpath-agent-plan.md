...
## Phase 3 ｜ personal_data 旧库迁移（本批最后做）✅ 脚本已交付（2026-07-12，待 NAS 执行）

> 实施与验收记录（本机只交付不连生产，代码为准）：
> - **`scripts/migrate_personal_data.py`**：一次性幂等可重跑。全部源行先落
>   import_raw source='legacy'（external_id='legacy-{表}-{id}'），归一化按
>   parse_status 门控——parsed 绝不重放，pending/failed/skipped 每次重试
>   （「无饮水习惯」这类 skip 建习惯后重跑可补）；单行失败 begin_nested 隔离，
>   失败原因写 parse_error。源连接强制只读；启动自检目标库迁移 12 已就位。
>   源表日期/主键列运行时从 information_schema 解析（候选列表兜底，解析不了
>   明确报错），--dry-run 完整试跑出报告后整体回滚
> - 字段映射照 §3.1 执行；开发时核对的定案：diet 查重口径同日+同文本+同 kcal
>   （kcal IS NOT DISTINCT FROM，NULL 也算同）、meal_type 未识别兜底加餐并列
>   报告；weight/围度按 mark_manual 语义（NULL 或 autofilled 可写并转手动，
>   目标已是手动值则保留新库、旧值弃并列报告）；neck_cm 无对应列 → 并入当日
>   notes；mood 越界（合法 1~10）记 failed 不兜底；target_kcal 取最新非空
>   target_calories 仅当现值为空；sex/birth_date/height_cm 仅当现值为空
> - **`scripts/freeze_personal_data.sql`**：方案 A REVOKE 写权限（DO 块自动扫
>   有写权限的角色逐个撤）+ 方案 B 改名 *_frozen（owner 即写入角色时用，
>   注释备用）；life_memories 不冻结
> - **`docs/legacy-migration-runbook.md`**：NAS 一页手册（备份 → dry-run 看报告
>   → 正式跑 → 核对 SQL/页面抽查 → 冻结；凭据占位执行时问用户）
> - 验收：tests/test_migrate_legacy.py 2 个端到端测试（同库 legacy_sim schema
>   模拟源，覆盖 meal 映射与兜底/diet 查重/weight 覆盖 autofill 不覆盖手动/
>   legacy-{id} 幂等/mood 越界隔离/饮水无匹配列报告+建习惯重跑补上/neck 并
>   notes/personal_info 仅空写入/重跑四遍幂等/dry-run 整体回滚），127 测全绿 +
>   CLI dry-run 冒烟实跑
> - 剩余（NAS 侧，照 runbook）：dry-run → 正式迁移 → 核对 → 冻结 →
>   删 Hermes 旧 skill 直写脚本（§3.4）
...
4. ~~**P3 迁移+冻结**~~ ✅ 脚本/冻结 SQL/手册已交付（见 §3 落地记录）；
   **NAS 执行待用户**（docs/legacy-migration-runbook.md：dry-run → 迁移 →
   核对 → 冻结）
