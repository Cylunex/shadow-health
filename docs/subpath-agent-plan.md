...
- [ ] dev 无前缀全量回归（92+ 测试全绿 + 手工走查今日/指标/饮食/训练/更多）
- [ ] 模板裸 URL lint 测试通过
- [ ] 本地起一个带 `X-Forwarded-Prefix: /shealth` 的 nginx/代理模拟走查全页面
- [ ] 真机：壳指向前缀地址，登录/打卡/拍照/秤/三星同步全链路
- [ ] `/shealth` 不带尾斜杠 301 到 `/shealth/`（nginx 段已含）
...
## Phase 2 ｜ 多 Agent 接入（REST 通道 + MCP server）
...
1. **P1 子路径适配** → 独立验收（dev 回归 + 前缀模拟走查）
2. **P2 REST+MCP+skill** → Hermes/OpenClaw 试记数笔、查询/周报可用
