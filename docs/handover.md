...
- NAS 实数核对：`sleep_sessions` 的 `timestamptz` 由 psycopg 以 UTC 返回，首页此前
  直接 `strftime`，会把本地 `23:15–07:54` 错显成 `15:15–23:54`。
- 今日概览现统一通过 `local_hm()` 转为 Asia/Shanghai；时长、入睡、起床都取
  `services.sleep.sessions_by_date()` 的同一优先来源。分段睡眠显示最早开始至最晚结束。

### 2026-08-13 S400 按需开秤可靠性修正

- NAS 日志确认服务端 7 天 14 条秤消息全部一次 200、parsed、无连续失败；异常场次发生在
  手机 BLE 扫描段（正常 12–26 秒，最慢 172/217 秒并需重复点击）。NAS 当前无 BLE
  网关进程，手机是唯一监听端。
- `ScaleScanService` 的 3 分钟称重模式改为 LOW_LATENCY + AGGRESSIVE，每次点击强制
  stop/start 重建扫描；常驻监听仍 LOW_POWER。`onScanFailed` 清理旧 callback，允许重试。
- S400 已捕获体重/低频阻抗后若先收到复位帧，立即提交已有测量，不再因高频末帧漏收而
  删除整次结果；网关监听器同步同一口径。保留 `neverForLocation`，避免扩大定位权限范围。

## ✅ 已完成：V14 趋势页与页面联动优化（2026-08-03）

1. 身体趋势页改为“当前状态 → 核心趋势 → 最近记录 → 数据工具”：首屏按指标独立取
   最近有效值和上次变化，常用图表只保留体重、体脂、睡眠、步数和静息心率，其他指标
   集中到下拉入口，空数据不再渲染无意义坐标轴
2. 修复 Chart.js 内联宽度把移动端单列网格撑到约 1000px 的问题；网格和图表容器统一
   `min-width: 0`，Canvas 限制最大宽度，390px 与桌面断点均无横向溢出
3. 移动端二级页面改为带明确父级的返回标题栏：食物库回饮食、训练工具回运动、导入中心
   回设置、报告详情回报告、秤状态回身体趋势，其余档案页回“我的”
4. 快速记录与趋势空态的 `#metrics-entry` 深链会自动展开补录区并滚动到位；秤状态页和
   习惯页补齐相关页面回链。首页顶部常驻“开秤”按钮：APK 内直接开启 3 分钟监听，
   普通浏览器回退到秤状态页。重建 Tailwind 并升级 SW v28

## ✅ 已完成：V13 全站响应式 UI 重构（2026-08-03）

1. 以深海蓝黑、紫色品牌光和数据类别色重建全局设计令牌；卡片、按钮、标签、输入框、
   进度条、列表、空态和登录页统一到一套组件，不改变 HTMX/Jinja 的服务端交互模式
2. 新增响应式应用骨架：桌面宽屏使用固定侧栏和顶部快速记录，移动端使用五栏底部导航
   与中央悬浮加号；快速记录面板直达餐照、饮食、指标、化验单、AI 和 Agent 流水
3. 信息架构从“录入入口”调整为“查看与核对”：今日页拆成体成分、活动、睡眠、饮食、
   自律和同步卡；趋势页桌面采用指标管理侧栏；饮食页增加四项快捷动作并把常用组合外置
4. “更多”页改为个人档案首页，按健康档案、洞察回顾、数据工具分组；训练降为运动记录，
   计划、动作库和体测留在二级入口；自律、化验、习惯、报告、AI、同步和设置统一页面标题
5. 主样式 URL 增加版本参数并升级 SW v26，防止旧 Service Worker 的 cache-first 策略继续
   下发旧 UI；全站主要路由做浏览器逐页检查和横向溢出检查

## ✅ 已完成：V12 识图溯源与页面减负（2026-08-03）

1. 迁移 16 给餐照、识图生成的饮食记录和化验指标增加 JSONB 溯源：只保存供应商、
   模型、主/独立识图配置来源、提示词版本、识别时间和置信度，不保存 API Key 或
   Base URL；化验单还保留识别原值与用户确认后的值，后续可审计修正差异
2. 餐照识别结果回写关联的饮食记录 ID；生成记录尚存时重复点击只回显既有结果，
   不再重复录入。若生成记录已全部删除，照片可重新识别
3. 饮食页餐次卡降低边框密度，餐照改为大缩略图状态条，直接区分“识别并录入”与
   “已识别 + 置信度”；识图生成的饮食行显示轻量来源标记
4. 化验档案改为查看优先：首页摘要展示最近报告、收录项和超范围项，拍照识别成为
   主入口，手工录入默认折叠；识别预览改成适合手机核对的逐项卡片，长期趋势突出
   最新值并保留历史与识图来源。Tailwind 重建，SW v25

## ✅ 已完成：V11 识图 LLM 独立配置（2026-08-03）

1. 新增 `app_settings['vision_llm_config']`，可单独选择 Claude / OpenAI 兼容端点，
   分别填写模型、API Key 和 Base URL；未启用独立识图时完整回退
   `app_settings['llm_config']`，旧部署无需迁移
2. `_call` 按是否携带图片自动选配置：餐照与化验单识别走视觉配置，健康分析、
   AI 问答和工具调用继续走主配置；视觉配置自身未配置 Key 时仍按供应商回退 `.env`
3. 设置页拆成“主模型 / 识图模型”，分别展示当前生效配置，并提供真实图片请求的
   识图连接测试

## ✅ 已完成：V10 “起飞”独立为自律 / 释放记录（2026-08-03）

NAS 实际数据确认三星直读会把自定义运动名原样写入 `session_type='起飞'`，并带
开始时间、时长、热量和平均/最高心率。它不是训练，不能污染运动次数、分钟、
负荷、准备度、报告和成就口径。

1. 新增 `release_logs` 与迁移 15：升级时按精确类型“起飞”（三星 zip 同时识别
   `detail.title`）把存量行从 `workout_logs` 原字段搬出；降级可无损搬回
2. 三星直读归一化新增双向分流：名称“起飞”写 `release_logs`，其他类型仍写
   `workout_logs`；同一三星 external_id 后续改名时会自动跨表搬移。三星 zip
   导入同样按原始标题分流。只做精确匹配，不误伤“起飞训练”等普通名称
3. 新增 `/discipline`：当前/最长自律天数、本月与累计释放数、90 天自律/释放
   日历、带时长/热量/心率的只读历史；首页概览与更多页均有入口
4. 设置导出及一键全量备份包含 `release_logs`；Tailwind 重建，SW v23；新增
   分流、改名、统计和页面回归锁。全量 302 测通过，390×844 移动端验证无横向
   溢出，日历与首页/更多入口正常

## ✅ 已完成：V9 查看优先改版（2026-07-30）

NAS 真实日志/生产数据审计结论：应用已进入「自动采集 + Agent 录入」阶段——近
30 天步数 30 天、心率 27 天、运动 27 天、体重 25 天；饮食 255 条中 Agent
写入 209 条，页面手工指标 POST 仅 6 次。访问侧首页 144 次、更多 95 次、指标
63 次、秤状态 42 次，但报告仅 2 次；原 UI 仍把大段录入表单放在查看内容之前，
与实际用法错位。

1. **首页查看优先**：新增 `/fragments/today/overview`，首屏直接展示最近/今日
   体重与较上次变化、体脂和来源、步数、训练、睡眠、当日热量/蛋白、体脂秤/
   三星健康/Agent 三通道最后成功时间；壳内保留首屏「接收体重 3 分钟」，并给
   趋势/日报/秤流水/Agent 流水直达入口
2. **录入默认折叠**：首页训练计划/习惯/指标/饮食/训练统一收进「需要时补录」，
   且片段改为 details 展开时触发 `lazy-load`，未展开不请求；指标、饮食、训练
   三个一级页的手工表单也默认折叠，查看内容紧跟其后
3. **体重回显污染修复**：生产审计确认已发生真实案例——前一天体重被表单
   回显带成当天手动值，随后当天体脂秤实测因「手动值不可覆盖」被挡住。现在
   历史体重只做 placeholder、不进入 value；当天已有体重
   显示为只读来源卡，明确点「修正」才启用有 name 的输入框；服务端所有相同值
   视为无变化，不重复写、不移除自动来源徽标
4. **趋势深链**：`/metrics?metric=steps|sleep&days=` 可直接打开指定图，不再
   一律回退体重图
5. **展示/录入指标独立控制**：指标页新增收起的「自定义指标」面板，分为
   「查看展示」与「手工录入」两个页签。`metrics_hidden_fields` 只控制趋势选项
   和历史表，新增 `metrics_entry_hidden_fields` 只控制指标表单与首页快速录入；
   旧版共用配置首次升级时复制为录入初值，随后两边独立保存、独立刷新。Tailwind
   重建，SW v22；回归锁覆盖旧配置迁移及两方向互不影响
...
1. **真机回归**（Android 壳与网关改动后必做；本次离线三阶段全是壳改动，装机必验）：
   ...
   - 秤「称重模式」：今日页/离线页/秤接收状态页点「⚖️ 开秤监听 3 分钟」→ 上秤 →
     ...
     仅在页面可见时每 3s 轮询 import_raw/sync_state，上秤后几秒内出带「新」标的记录。
     首页开秤会启动一次 195 秒会话，直接回显「监听中 → 已收到 → 体重/体成分已入库」，
     成功或超时后自动停止轮询并刷新今日概览；排查秤问题再开状态页
