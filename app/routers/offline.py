...
def _normalize_diet(db: Session, payload: dict, d: date) -> int:
    log = DietLog(log_date=d, **parse_diet_payload(payload))
    db.add(log)
    db.flush()
    return log.id
...
def _normalize_metric(db: Session, payload: dict, d: date) -> int | None:
...
async def ingest_batch(
    ...
    def _resp(new: int, skipped: int, results: list[dict[str, Any]]) -> JSONResponse:
